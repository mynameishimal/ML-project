...
random.seed(0)
torch.manual_seed(0)
log_filename='cutout,GB,RHF_resnet18'
PATH = 'models/pneumonia.model'
...
#def imshow(img):
    ...
    #plt.show()
#images = iter(trainloader).next()[0]
...
def has_best_performance(dictionary_performances, key, lowest, size_limit=3):
...
for epoch in range(epoch_runs):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        trainingAccuracy = accuracy_score(y_true, y_pred)
        ...
        writer.add_scalar('training accuracy', trainingAccuracy)
        ...
        if i%10==0:
            ...
            validation_performance = add_performance(validation_performance, "validation_accuracy", v_accuracy)
            writer.add_scalar('validation_accuracy', v_accuracy, len(trainloader)*epoch + i)
            ...
            if is_best:
                ...
                file_handlers.write('validation loss:'+str(mean(validation_performance['validation_loss'])))
                file_handlers.write('covid19 negative f1:'+str(mean(validation_performance['covid19_negative_fscore'])))
