...
random.seed(0)
torch.manual_seed(0)
log_filename='network_alexnet_squaredWeights_weightedBatch'
writer  = SummaryWriter('modelRuns/'+log_filename)
...
train_sampler = torch.utils.data.sampler.WeightedRandomSampler(weights = train_set_sample_weights, num_samples=len(train_set_sample_weights))
...
test_sampler = torch.utils.data.sampler.WeightedRandomSampler(weights = test_set_sample_weights, num_samples=len(test_set_sample_weights))

trainloader = torch.utils.data.DataLoader(trainset, batch_size=4, sampler=train_sampler)
testloader = torch.utils.data.DataLoader(testset, batch_size=4, sampler=test_sampler)
